...
- `::hint` renders folded by default; `::tip`, `::note` and `::warn`
  render always-visible callouts (violet / neutral / amber) - tips for
  terminal-technique notes (Tab completion, history keys) that checks
  cannot verify, notes for asides, warnings for destructive or surprising
  behavior; `::image` and plain `![...]()` serve unit-local files.
